...
def get_sku_orders():
    sql = '''
    SELECT 
    所属部门,客户ID,订单编号,商品名称,商品简称,商品类型,规格,商品编号,数量,中心,部门,小组,
...
def save_sql(df):
    ...
    INSERT INTO `dai_sku_orders` 
     (`id`,`dept_name`,`member_id`,`order_sn`,`product_name`,
     ...
     ON DUPLICATE KEY UPDATE
         `dept_name`= VALUES(`dept_name`),`member_id`= VALUES(`member_id`),`order_sn`=VALUES(`order_sn`),
