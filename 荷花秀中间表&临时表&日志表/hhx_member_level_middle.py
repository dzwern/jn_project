...
def main():
    ...
    df_hhx_member = df_hhx_member.fillna(0)
    print(df_hhx_member)
    df_hhx_member['last_time'] = df_hhx_member['last_time'].apply(lambda x: None if x == 0 else x)
    df_hhx_member['last_time'] = df_hhx_member['last_time'].astype(str).replace('NaT', None)
    del_sql()
