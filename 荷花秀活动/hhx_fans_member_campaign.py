...
def get_user_fans():
    ...
    GROUP BY a.dept_name,a.member_category
...
def save_sql(df):
    ...
    INSERT INTO `t_fans_member_campaign` 
     (`id`,`dept_name1`,`dept_name2`,`dept_name`,`wechat_nums`,
     `member_category`,`fans`,`members_develop`,`member_rate`,`members_amount`,
     `member_price`,`activity_name`
     ) 
     VALUES (
     %s,%s,%s,%s,%s,
     %s,%s,%s,%s,%s,
...
def main():
    ...
    df_user_base = df_user_base.merge(df_user_fans, on=['dept_name'], how='left')
    # 员工真实消费情况
    ...
    df_user_base = df_user_base.merge(df_member_strike, on=['dept_name', 'member_category'], how='left')
    # 分类汇总
    df_user_base1 = df_user_base[['dept_name1', 'dept_name2', 'dept_name',
                                  'wechat_nums', 'member_category', 'fans']].drop_duplicates()
    df_user_base2 = df_user_base.groupby(["dept_name", "member_category"])[
        ['members_develop', 'members_amount']].sum().reset_index()
    df_user_base = df_user_base1.merge(df_user_base2, on=['dept_name', 'member_category'], how='left')
    # 转化率
    ...
    df_user_base['id'] = df_user_base['dept_name'].astype(str) + df_user_base['member_category'].astype(str) + \
                         df_user_base['activity_name']
    df_user_base = df_user_base[['id', 'dept_name1', 'dept_name2', 'dept_name',
...
if __name__ == '__main__':
    ...
    # 2023年51活动前客户等级，2023年38女神节活动前客户等级，2023年618活动前客户等级
